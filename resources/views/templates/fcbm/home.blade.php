        ...
        <div class="max-w-4xl mx-auto bg-white bg-opacity-10 backdrop-blur-lg rounded-2xl p-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                ...
                <div class="text-center">
                    @if($settings->logo)
                        <img src="{{ asset('storage/' . $settings->logo) }}" alt="{{ $settings->website_name }}" class="h-24 w-24 mx-auto mb-4">
                    @else
                        <div class="h-24 w-24 mx-auto mb-4 bg-white rounded-full flex items-center justify-center">
                            <span class="text-primary font-bold text-3xl">{{ substr($settings->website_name ?? 'FC', 0, 2) }}</span>
                        </div>
                    @endif
                    <h3 class="text-2xl font-bold">{{ $settings->website_name }}</h3>
                </div>

                <!-- Match Info -->
                <div class="text-center">
                    <div class="text-6xl font-bold mb-4">VS</div>
                    <div class="text-xl mb-2">Samstag, {{ now()->addDays(5)->format('d.m.Y') }}</div>
            ...
            <table class="w-full">
                ...
                <tbody>
                    <tr class="border-b hover:bg-gray-50 bg-primary bg-opacity-5">
                        ...
                        <td class="py-4 px-6 font-bold text-primary">{{ $settings->website_name }}</td>
