...
echo "=== Dropping comet_facility_fields and comet_facilities from all tenants ===\n\n";
...
foreach ($tenants as $tenant) {
    ...
    try {
        ...
        // Drop comet_facility_fields first
        ...
        DB::connection('tenant')->statement('DROP TABLE IF EXISTS comet_facility_fields');
        echo "  ✓ Dropped comet_facility_fields\n";

        // Drop comet_facilities
        $fks = DB::connection('tenant')->select("
            SELECT CONSTRAINT_NAME, TABLE_NAME
            FROM information_schema.KEY_COLUMN_USAGE
        ...
        DB::connection('tenant')->statement('DROP TABLE IF EXISTS comet_facilities');
